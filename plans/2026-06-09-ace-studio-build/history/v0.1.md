...
1. **M1 音檔庫先用 localStorage 可以嗎？** 建議可以（最快走通，metadata 一樣完整記錄），落地 `library.json` 放 M3 一起做。
2. **本機代理用 Node(Express)？** 建議是（與前端同生態、開檔總管用 child_process、之後接 Claude 都順）。M3 才需要。
3. **M1 結束後下一步做哪個？** 預設依序 M2 → M3…；若你想先要「能打開檔案總管 + library.json 落地」可把 M3 提前。
