...
5. **M5 — AI 接口（MCP）**：`mcp-server/` 讓 Claude Code 等 MCP 客戶端自然語言驅動生成（見 §17）。✅*（2026-06-11 拍板：MCP 取代內建聊天；M5b 內建聊天面板視使用情況再議）*
...
| GET | `/library` | — | `{ok, items, dir, updatedAt}` | 讀 `library/library.json`（`updatedAt` 供前端輪詢比對） |
| POST | `/library` | `{items: LibraryItem[]}` | `{ok}` | 整份覆寫 library.json |
| POST | `/library/add-item` | `{item: LibraryItem}` | `{ok}` | **單筆插入**（伺服器端原子讀寫；同 id 取代。M5：MCP 與前端共用） |
| POST | `/library/remove-item` | `{id}` | `{ok, removed}` | **單筆移除** + 連帶刪其 `library/audio/` 音檔 |
...
3. 增刪走 `/library/add-item`、`/library/remove-item`（M5 起；伺服器端讀最新檔再寫，前端與 MCP 不互相蓋寫）
4. 前端啟動時 `hydrate()`：磁碟有資料→以磁碟為準；磁碟空而 localStorage 有舊作品→**自動遷移**（複製音檔 + 寫 library.json）
5. 前端每 5 秒（視窗在前景時）輪詢 `GET /library` 比對 `updatedAt`——MCP 生成入庫後 app 自動刷新
6. run-local 沒開 → 自動退回 localStorage 模式（行為同 M2）

## 17. MCP 接口（`mcp-server/`，M5）

讓 Claude Code / 任何 MCP 客戶端用自然語言驅動生成。`.mcp.json` 在專案根目錄，Claude Code 於本資料夾開啟時自動載入（首次需 `cd mcp-server && npm install`）。規劃見 `plans/2026-06-11-m5-ai-interface/`。

| 工具 | 參數 | 行為 |
|---|---|---|
| `generate_bgm` | prompt, duration(5–120)=60, lyrics?, seed?, trim=true | 自動 init 引擎（若未初始化）→ 先 `/sfx/release` 還 VRAM → release_task → 輪詢（9 分鐘上限）→ 裁切 → 入庫 |
| `generate_sfx` | prompt, duration(0.5–8)=1.5, seed?, trim=true | `:8002 /generate`（steps=100, cfg=7）→ 裁切 → 入庫 |
| `list_library` | type?(bgm/sfx) | 列作品（id/標題/類型/長度/prompt/路徑） |
| `remove_item` | id | helper `/library/remove-item`（連音檔一起刪） |
| `studio_status` | — | 三服務健康 + 已載入模型 + 啟動提示 |

- 入庫走與 UI 完全相同的管線（trim → import-audio → add-item），前端輪詢自動顯示。
- 引擎單併發（workers=1）：AI 做「音效包」= 連續呼叫多次，自然排隊。
- 冒煙測試：`node mcp-server/smoke-test.mjs`（協定 + 狀態）；`--full` 加跑真實 SFX 生成。
